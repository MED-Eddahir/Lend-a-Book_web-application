  ...
  retrieveLoan(issueId){
   ...
   addCollection(name) {
        if (!this.collections[name]) {
            this.collections[name] = []; // array di issueId
            return true;
        }
        return false;
    }

    addBookToCollection(name, issueId) {
        if (this.collections[name] && !this.collections[name].includes(issueId)) {
            this.collections[name].push(issueId);
            return true;
        }
        return false;
    }
...
userBookshelves["Andrea"].addOwnedBook("2-10", "1-14")
